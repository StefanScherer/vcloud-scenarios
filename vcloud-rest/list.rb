...
puts "### Fetch and Show #{orgname} Organization"
org = connection.get_organization(orgs[orgname])
ap org

### Fetch and show a vDC, OvDC-PAYG-Bronze-01 is an example, you should replace it with your own vDC

vdcname = org[:vdcs].keys[0]

puts "### Fetch and Show #{vdcname} vDC"
vdc = connection.get_vdc(org[:vdcs][vdcname])
ap vdc

# ### Fetch and show a Catalog, Vagrant is an example, you should replace it with your own Catalog
catalogname = org[:catalogs].keys[0]

puts "### Fetch and Show #{catalogname} Catalog"
cat = connection.get_catalog(org[:catalogs][catalogname])
ap cat
